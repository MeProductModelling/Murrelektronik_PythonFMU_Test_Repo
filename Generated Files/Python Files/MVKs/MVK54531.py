...
class MVK54531(Fmi3Slave):
    """
    This class is a main entry point to declare IOs, parameters and Simulation related variables for this MVK 54531 device.
    (Who and whom.. TODO)
    ...
    def __init__(self, **kwargs):
        ...
        # Parameters Pin, port, Compact IOs

        # IMPORTANT: Group the variables into different categories and encapsulate them into classes.

        # Variables for Parameters of the Module
        self.parameters = Parameters()
        self.parameters.register(self)

        # Variables for sensor variables..
        self.sensor_inputs = SensorInputs()
        self.sensor_inputs.register(self)

        # Variables for actuator inputs...
        self.actuator_outputs = ActuatorOutputs()
        self.actuator_outputs.register(self)

        # Variables which shall be connected to PLC to exchange data from Simulation Model.
        self.plc_ios = PLCIOs()
        self.plc_ios.register(self)

        # Variables that can be observed and changed by User when an User Interface uis generated to this Simulation model in different Simulation tools.
        self.user_interface_ios = UserInterfaceIOs()
        self.user_interface_ios.register(self)

        # Variables for time is necessary calculate values in model based in time.
        self.author = "Raj Kumar"
        self.description = (
            "A simple description of DIO 8 module from Murrelektronik catalogue"
        )
        self.time = 0.0
    ...
    def do_step(self, current_time, step_size):
        ...
        def _get_qualifier_di_bytes():
            ...
            (self.plc_ios.qualifier_di_8_byte1, self.plc_ios.qualifier_di_8_byte2) = (
                qualifier_di_8(
        ...
        def _get_qualifier_do_bytes():
            ...
            (self.plc_ios.qualifier_do_8_byte1, self.plc_ios.qualifier_do_8_byte2) = (
                qualifier_do_8(
                    self.plc_ios.inByte1,
                    self.plc_ios.inByte2,
        ...
        def _get_system_state_bytes():
            ...
            (
                self.plc_ios.system_state_byte_1,
                self.plc_ios.system_state_byte_2,
                self.plc_ios.system_state_byte_3,
                self.plc_ios.system_state_byte_4,
            ) = System_State(
                self.plc_ios.qualifier_di_8_byte1,
                self.plc_ios.qualifier_di_8_byte2,
                self.plc_ios.qualifier_do_8_byte1,
                self.plc_ios.qualifier_do_8_byte2,
                False,
                False,
